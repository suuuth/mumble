...
Poke::Poke(QWidget *p, QString title, bool bChannel) : QDialog(p) {
	setupUi(this);
	if (!bChannel)
...
void Poke::on_qcbTreeMessage_stateChanged(int s) {
	bTreeMessage = s == Qt::Checked ? true : false;
}

QString Poke::autoFormat(QString qsPlain) {
	QRegExp qr;
	qr.setMinimal(true);
